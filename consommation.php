...
<html>
  ...
  <body>
            ...
            <table id="myTable" class="table table-striped table-bordered">
              <thead class="thead-dark">
                <tr>
                  <th>Date</th>
                  <th>Consommation</th>
                  <th>Max 00:00 -> 07:00</th>
                  <th>Max 07:00 -> 17:00</th>
                  <th>Max 17:00 -> 22:00</th>
                  <th>Max 22:00 -> 24:00</th>
                </tr>
              </thead>
              <tbody id="myTbody">
                <?php
    ...
    <script type='text/javascript'>
      ...
      const findMaxInRange = (valuesArray, startHour, endHour) => {
          return Math.max(0,...valuesArray
              .filter(item => {
      ...
      for (const ele of dataArray){
        currentDate = ele.timestamp.substring(0,10) 
        if ( currentDate in newValues){
          newValues[currentDate].push(ele)
        }else {
          newValues[currentDate] = [ele]
        }
      }
      for (const [date, value] of Object.entries(newValues)) {
        const phase0_7 = findMaxInRange(value,0,7)
        const phase7_17 = findMaxInRange(value,7,17)
        const phase17_22 = findMaxInRange(value,17,22)
        const phase22_24 = findMaxInRange(value,22,24)
        const Umax = 400
        const consommation = (Umax * (Math.sqrt(3)/2000) * 0.987 * (phase0_7*0.64*7+phase7_17*0.88*10+phase17_22*1.24*5+phase22_24*0.64*2)).toFixed(2)
        values.push(consommation)
    ...
    <script>
      function exportData() {
        ...
        XLSX.writeFile(workbook, 'Consommation.xlsx');
      }
